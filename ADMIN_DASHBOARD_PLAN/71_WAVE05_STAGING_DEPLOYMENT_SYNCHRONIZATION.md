# 71_WAVE05_STAGING_DEPLOYMENT_SYNCHRONIZATION

**Document ID:** 71_WAVE05_STAGING_DEPLOYMENT_SYNCHRONIZATION  
**Date:** 2026-07-22  
**Project:** VietSalePro  
...
**Wave:** Wave-05  
**Acting Capacity:** Enterprise Program Management Office (PMO) / Supabase Deployment Engineer  
...
**Status:** Wave-05 Staging Deployment Synchronization COMPLETE — **STAGING ONLY**

------------------------------------------------------------------------

## 1. Executive Summary

This document records the Stage 71 Wave-05 Staging Deployment Synchronization for the Admin Dashboard System Remediation Program. It is a controlled deployment synchronization gate only. It does **not** authorize production deployment, source modification, database changes, or the start of Stage 72.

The sole authorized Wave-05 artifact — the corrected `billing-webhooks` Edge Function (`supabase/functions/billing-webhooks/index.ts`) — has been synchronized to the authorized **STAGING** Supabase project `shbmzvfcenbybvyzclem` (QLBH Staging Multi-Tenant). The Vercel production deployment remains unchanged. No frontend, database, RPC, migration, service, permission, or secret changes were performed.

**Deployment Decision:**

``` text
WAVE-05 STAGING DEPLOYMENT SYNCHRONIZATION COMPLETE — STAGING ONLY
```

**Production Deployment Decision:**

- Wave-05 Production Deployment Synchronization document creation: **NOT AUTHORIZED**.
- Wave-05 Production Deployment Synchronization execution: **NOT AUTHORIZED**.
- Stage 72 execution: **NOT AUTHORIZED** until explicit Program Owner approval is received.
...
| Check | Method | Result |
|-------|--------|--------|
| Project accessibility | `supabase-mcp-server` `get_project` | `shbmzvfcenbybvyzclem` `ACTIVE_HEALTHY`, Postgres 17.6.1.141 |
| Authentication | `supabase-mcp-server` `list_projects` | Accessible; authorized to both staging and production projects |
| Existing staging `billing-webhooks` | `supabase-mcp-server` `list_edge_functions` | v1 `ACTIVE`, `verify_jwt: false` |
| Existing source (remote) | `supabase-mcp-server` `get_edge_function` | Remote source still imported `decodeBase64` (pre-implementation version) |
...
| Deployed function | `billing-webhooks` |
| `verify_jwt` preserved | `false` (confirmed in `supabase/config.toml` and post-deploy MCP verification) |

### 7.3 Post-Deployment Verification

| Check | Method | Result |
|-------|--------|--------|
| Function version | `supabase-mcp-server` `list_edge_functions` | `billing-webhooks` v2 `ACTIVE`, `verify_jwt: false` |
| Remote source | `supabase-mcp-server` `get_edge_function` | `import { decode as decodeBase64 }` present; `decodeBase64` call site unchanged |
| Endpoint availability | `curl.exe -i -X OPTIONS` | `HTTP/1.1 200 OK` with CORS headers |
| Runtime smoke | `curl.exe -X POST ?provider=momo` | `HTTP/1.1 200 OK` with valid JSON response; no `BOOT_ERROR` or `503` |

**Supabase Deployment Verdict:** The `billing-webhooks` Edge Function was successfully redeployed to staging. `verify_jwt` remains `false`, and the corrected Deno std import now boots and responds correctly.
...
| Implementation commit (source change) | `d554dda0` (`fix(supabase/functions,68,68A,00,12): Wave-05 Implementation - billing-webhooks decodeBase64 alias`) |
| Governance commit | `969b832a` (`docs(70,70A,00,12): Wave-05 Acceptance Review`) |
...
| Staging deployment drift | Post-deploy `get_edge_function` vs. local source | Remote source matches local source on line 13 import |
